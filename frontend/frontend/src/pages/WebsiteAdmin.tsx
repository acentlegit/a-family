...
const WebsiteAdmin: React.FC = () => {
  ...
  const generateWebsiteWithAI = async () => {
    ...
    console.log('🤖 AI generation started - this may take 2-10 minutes...');
    
    // Show periodic updates to user
    let updateInterval: NodeJS.Timeout | null = null;
    updateInterval = setInterval(() => {
      const elapsed = Math.floor((Date.now() - startTime) / 1000);
      const minutes = Math.floor(elapsed / 60);
      const seconds = elapsed % 60;
      console.log(`⏳ AI generation in progress... ${minutes}m ${seconds}s elapsed`);
    ...
    try {
      ...
      // AI generation can take 2-10 minutes, so use much longer timeout
      // Use axios directly with extended timeout for this long-running request
      const apiUrl = getApiUrl();
      const token = localStorage.getItem('token');
      
      // Show a more informative loading message
      console.log('🤖 Starting AI generation - this may take 2-10 minutes...');
      ...
      const response = await axios.post(
        ...
        {
          customerDetails: {
            // Send all form values so Ollama can use them
            siteTitle: siteTitle,
        ...
        {
          timeout: 600000, // 10 minutes timeout for AI generation (increased from 5 minutes)
          headers: {
      ...
      if (response.data.success) {
        ...
        const elapsed = Math.floor((Date.now() - startTime) / 1000);
        console.log(`✅ AI generation completed in ${Math.floor(elapsed / 60)}m ${elapsed % 60}s`);
        ...
        alert(`✅ Website generated successfully!\n\nGenerated ${response.data.pagesCreated || response.data.pages?.length || 0} pages in ${Math.floor(elapsed / 60)}m ${elapsed % 60}s.`);
      } else {
        if (updateInterval) clearInterval(updateInterval);
      ...
      // Handle network errors (including ERR_NETWORK_IO_SUSPENDED)
      if (
        ...
        const networkErrorMessage = 
          '🌐 Network connection error. The request may have been suspended by the browser.\n\n' +
          'This can happen with long-running requests (AI generation takes 2-10 minutes).\n\n' +
          'Please try:\n' +
          '1. Check if the backend is still running\n' +
          '2. Check backend console for generation progress\n' +
          '3. Wait a few minutes and refresh the page - generation may have completed\n' +
          '4. Try again with a smaller number of pages\n' +
      ...
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout') || error.message?.includes('exceeded')) {
        const timeoutMessage = 
          ...
          'This is normal for AI generation (can take 2-10 minutes).\n\n' +
          'Possible reasons:\n' +
          '• Ollama is processing a large request with extensive content\n' +
          '• Your computer may be slow\n' +
          '• Ollama model needs to be downloaded\n' +
          '• Generating multiple pages with detailed content\n\n' +
          'Please check:\n' +
          '1. Is Ollama running? (ollama serve)\n' +
          '2. Check backend console for progress - generation may still be running\n' +
          '3. Wait a few more minutes and check if pages were created\n' +
          '4. Try again if needed - the backend may have completed the generation';
        
        // Show a more user-friendly dialog
        if (window.confirm(timeoutMessage + '\n\nClick OK to continue, or Cancel to wait longer.')) {
          // User clicked OK - reload pages to check if generation completed
          console.log('🔄 Checking if generation completed...');
          await loadPages();
          const updatedPages = await api.get(`/website-admin/pages/${selectedFamilyId}`);
          if (updatedPages.data.success && updatedPages.data.pages && updatedPages.data.pages.length > pages.length) {
            alert('✅ Good news! The website was generated successfully. Pages have been created.');
            await loadConfig();
            await loadPages();
            await generatePreview();
          }
    ...
    try {
      ...
      if (response.data.success && response.data.config) {
        ...
        if (dbConfig.logo_url || dbConfig.logoUrl) {
          ...
          // Always construct URL based on current API base URL to avoid CORS issues
          let imagePath = logoUrl;
          if (logoUrl.startsWith('http://') || logoUrl.startsWith('https://')) {
            // Extract path from full URL to use current API base
            try {
              const urlObj = new URL(logoUrl);
              imagePath = urlObj.pathname;
            } catch (e) {
              // If URL parsing fails, try to extract path manually
              const pathMatch = logoUrl.match(/\/uploads\/.+$/);
              if (pathMatch) {
          ...
          // Construct full URL using current API base (local or production)
          const baseUrl = getApiUrl().replace('/api', '');
          const fullLogoUrl = imagePath.startsWith('/') 
            ? `${baseUrl}${imagePath}` 
            : `${baseUrl}/${imagePath}`;
          ...
          <button 
            ...
            style={{ 
              background: aiGenerating 
                ? 'linear-gradient(135deg, #012a55 0%, #001F3F 100%)'
                : '#001F3F',
              color: 'white',
              marginRight: '10px',
              position: 'relative',
            ...
            <div className="form-group">
                    ...
                    if (file && selectedFamilyId) {
                      ...
                      try {
                        ...
                        if (uploadResponse.data.success) {
                          ...
                          // Always construct URL based on current API base URL to avoid CORS issues
                          // Extract the path from the returned URL (e.g., /uploads/website-logos/logo-xxx.jpg)
                          let imagePath = logoUrl;
                          if (logoUrl.startsWith('http://') || logoUrl.startsWith('https://')) {
                          ...
                          // Update both config and preview to ensure logo stays visible
                          // Use functional update to ensure state persists
                          setConfig(prevConfig => ({ ...prevConfig, logoUrl: fullLogoUrl }));
                          setLogoPreview(fullLogoUrl);
                          console.log('✅ Logo uploaded successfully:', fullLogoUrl);
                          console.log('✅ Logo preview URL:', fullLogoUrl);
                          console.log('✅ Config logoUrl will be:', fullLogoUrl);
                          
                          // Show success message without blocking - use setTimeout to ensure state is set first
                          setTimeout(() => {
              ...
              {(logoPreview || config.logoUrl) && (
                <div className="logo-preview">
                  <img 
                    key={`logo-${logoPreview || config.logoUrl}`}
                    src={logoPreview || config.logoUrl} 
                    alt="Logo preview" 
                    className="logo-preview-img"
                    onError={(e) => {
                      // Handle image loading errors gracefully - try to fix URL first
                      ...
                      // Try alternative URL formats before showing placeholder
                      if (retryCount < 3) {
                        let alternativeUrl = currentSrc;
                        
                        // If URL contains production domain, convert to current API base URL
                        if (currentSrc.includes('api.arakala.net') || currentSrc.includes('arakala.net')) {
                          const baseUrl = getApiUrl().replace('/api', '');
                          const pathMatch = currentSrc.match(/\/uploads\/.+$/);
                          if (pathMatch) {
                            alternativeUrl = `${baseUrl}${pathMatch[0]}`;
                            console.log('🔄 Retrying with local URL:', alternativeUrl);
                          }
                        } else if (currentSrc.includes('?')) {
                          // Try removing query parameters if any
                          alternativeUrl = currentSrc.split('?')[0];
                        } else if (!currentSrc.startsWith('http')) {
                          // Try constructing full URL
                          const baseUrl = getApiUrl().replace('/api', '');
                          alternativeUrl = currentSrc.startsWith('/') 
                            ? `${baseUrl}${currentSrc}` 
                            : `${baseUrl}/${currentSrc}`;
                        }
                      ...
                      // All retries failed, but don't hide immediately - keep trying with state URL
                      const parent = target.parentElement;
                      
                      // Don't hide the image immediately - it might load on next render
                      // Only show placeholder if image is truly unavailable
                      if (retryCount >= 3) {
                        target.style.display = 'none';
                        
                        // Remove existing placeholder if any
                        const existingPlaceholder = parent?.querySelector('.logo-error-placeholder');
                        if (existingPlaceholder) {
                          existingPlaceholder.remove();
                        }
                        
                        // Create error placeholder only after all retries
                        if (parent && !parent.querySelector('.logo-error-placeholder')) {
                          const placeholder = document.createElement('div');
                          placeholder.className = 'logo-error-placeholder';
                          placeholder.style.cssText = 'width: 100px; height: 100px; border-radius: 50%; background: #f0f0f0; display: flex; align-items: center; justify-content: center; color: #999; font-size: 11px; text-align: center; padding: 10px; border: 2px dashed #ddd;';
                          placeholder.textContent = 'Logo not available';
                          parent.insertBefore(placeholder, target);
                        }
                      }
                    }}
                    onLoad={(e) => {
                      // Hide any error placeholders when image loads successfully
                      const target = e.target as HTMLImageElement;
                      const parent = target.parentElement;
                      const placeholder = parent?.querySelector('.logo-error-placeholder');
                      if (placeholder) {
                        placeholder.remove();
                      }

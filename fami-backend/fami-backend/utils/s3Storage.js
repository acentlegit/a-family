...
const uploadToS3 = async (fileBuffer, originalName, mimeType, user = null, familyName = null, eventName = null) => {
  ...
  try {
    ...
    // Use path-style URL if bucket name contains dots (required for SSL)
    ...
    let url;
    if (bucket.includes('.')) {
      // Bucket name contains dots - use path-style URL to avoid SSL certificate issues
      url = `https://s3.${region}.amazonaws.com/${bucket}/${key}`;
    } else {
      // Use virtual-hosted style for bucket names without dots

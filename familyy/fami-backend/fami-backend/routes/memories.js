...
router.get('/:familyId', protect, async (req, res) => {
  try {
    ...
    // Process each memory to generate presigned URLs for S3 media
    const processedMemories = await Promise.all(
      memories.map(async (memory) => {
        if (memory.media && Array.isArray(memory.media)) {
          const processedMedia = await Promise.all(
            memory.media.map(async (mediaItem) => {
              let mediaUrl = mediaItem.url;
              let thumbnailUrl = mediaItem.thumbnail || mediaItem.url;
              
              // If media is stored in S3, generate presigned URL
              const isS3Url = mediaUrl && mediaUrl.includes('amazonaws.com');
              
              if (isS3Configured && (mediaItem.source === 's3' || isS3Url)) {
                let s3Key = mediaItem.s3Key;
                
                // If s3Key is not stored, extract it from the URL
                if (!s3Key && mediaUrl) {
          ...
          return {
            ...memory.toObject ? memory.toObject() : memory,
            media: processedMedia
          };
        }
        
        return memory.toObject ? memory.toObject() : memory;
      })
    );

    res.json({ success: true, count: processedMemories.length, data: processedMemories });
  } catch (error) {

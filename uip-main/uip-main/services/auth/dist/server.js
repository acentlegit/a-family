...
(function (Role) {
    ...
    Role["BUSINESS_ADMIN"] = "business_admin";
    Role["USER"] = "user";
...
(function (Permission) {
    // Tenant Management
    Permission["CREATE_TENANT"] = "create_tenant";
    Permission["MANAGE_ALL_TENANTS"] = "manage_all_tenants";
    // User Management
    Permission["MANAGE_ALL_USERS"] = "manage_all_users";
    Permission["MANAGE_TENANT_USERS"] = "manage_tenant_users";
    // Role Assignment
    Permission["ASSIGN_ROLES"] = "assign_roles";
    Permission["ASSIGN_TENANT_ROLES"] = "assign_tenant_roles";
    // Intent Configuration
    Permission["CONFIGURE_INTENTS"] = "configure_intents";
    Permission["CONFIGURE_TENANT_INTENTS"] = "configure_tenant_intents";
    // Dashboards
    Permission["VIEW_ALL_DASHBOARDS"] = "view_all_dashboards";
    Permission["VIEW_TENANT_DASHBOARDS"] = "view_tenant_dashboards";
    Permission["VIEW_LIMITED_DASHBOARDS"] = "view_limited_dashboards";
    // Integrations
    Permission["MANAGE_GLOBAL_INTEGRATIONS"] = "manage_global_integrations";
    Permission["MANAGE_TENANT_INTEGRATIONS"] = "manage_tenant_integrations";
    // Billing
    Permission["MANAGE_PLATFORM_BILLING"] = "manage_platform_billing";
    Permission["MANAGE_TENANT_BILLING"] = "manage_tenant_billing";
    // Audit Logs
    Permission["VIEW_ALL_AUDIT_LOGS"] = "view_all_audit_logs";
    Permission["VIEW_TENANT_AUDIT_LOGS"] = "view_tenant_audit_logs";
    // Cross-tenant Access
    Permission["CROSS_TENANT_ACCESS"] = "cross_tenant_access";
...
app.get("/users/me/permissions", async (req, reply) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return reply.code(401).send({ error: "Unauthorized" });
...
app.post("/case-file/save", async (req, reply) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return reply.code(401).send({ error: "Unauthorized" });
...
app.get("/case-file/history", async (req, reply) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return reply.code(401).send({ error: "Unauthorized" });

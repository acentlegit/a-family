...
function makeDigest(session) {
    ...
    if (session.latestDecision)
        text.push(`Decision: ${session.latestDecision.status} — ${session.latestDecision.reason}`);
    if (session.explanation)
        text.push(`Explanation: ${session.explanation}`);
    text.push("");
    text.push("State Objects:");
    for (const o of objs) {
        const status = o.status ?? o.severity ?? "";
        text.push(`- ${o.kind}: ${o.title} (${status})`);
    }
    text.push("");
    text.push("Recent Events:");
    for (const e of recent) {
        if (e.type === "IntentReceived")
            text.push(`- Intent received: ${e.envelope.intentType}`);
        if (e.type === "EngineResult")
            text.push(`- Engine ${e.result.engine}: ${e.result.status} — ${e.result.summary}`);
        if (e.type === "Decision")
            text.push(`- Decision: ${e.decision.status} — ${e.decision.reason}`);
        if (e.type === "Evidence")
            text.push(`- Evidence: ${e.artifact.title}`);
        if (e.type === "Note")
            text.push(`- Note: ${e.text}`);
    }
  ...
  <div class="card">
    ...
    <ul>
      ${recent
        .map((e) => {
        if (e.type === "IntentReceived")
            return `<li><b>Intent</b>: ${e.envelope.intentType}</li>`;
        if (e.type === "EngineResult")
            return `<li><b>${e.result.engine}</b>: ${e.result.status} — ${e.result.summary}</li>`;
        if (e.type === "Decision")
            return `<li><b>Decision</b>: ${e.decision.status} — ${e.decision.reason}</li>`;
        if (e.type === "Evidence")
            return `<li><b>Evidence</b>: ${e.artifact.title}</li>`;
        if (e.type === "Note")
            return `<li><b>Note</b>: ${e.text}</li>`;
        return `<li>Event</li>`;
    })
        .join("")}
    </ul>

...
exports.generatePovOptions = generatePovOptions;
exports.generateSessionTypes = generateSessionTypes;
...
function sanitizeSummaryDetails(intentText, summary) {
    ...
    const containsValue = (value) => {
        ...
        if (typeof value === "object") {
            if (typeof value.id === "string")
                return containsValue(value.id);
            if (typeof value.name === "string")
                return containsValue(value.name);
        }
        return false;
    };
    const filtered = {};
 ...
 */
async function generatePovOptions(intentText, industry) {
    const prompt = `USER INTENT: "${intentText}"
INDUSTRY: ${industry}
 ...
 */
async function generateSessionTypes(intentText, industry) {
    const prompt = `USER INTENT: "${intentText}"
INDUSTRY: ${industry}
...
function looksLikeSchema(response) {
...
function extractStringsFromParsed(parsed, preferredKeys) {
    const items = [];
    const pushItem = (value) => {
        ...
        if (value && typeof value === "object") {
            ...
            if (typeof value.step === "string")
                items.push(value.step);
            if (typeof value.role === "string")
                items.push(value.role);
            if (typeof value.type === "string")
                items.push(value.type);
        }
    };
    if (Array.isArray(parsed)) {
        ...
        if (!items.length && Array.isArray(parsed.items)) {
            parsed.items.forEach(pushItem);
        }
    }
    return Array.from(new Set(items.map((item) => String(item).trim()))).filter((item) => item.length > 0);
}
/**

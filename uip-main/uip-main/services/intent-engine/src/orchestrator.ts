...
import { generateEngineResponse, generateActionItems, generateSummaryDetails, generatePovOptions, generateSessionTypes } from "./ollama-service";

const OLLAMA_ONLY = process.env.OLLAMA_ONLY !== 'false';

async function processIntent(req: IntentRequest): Promise<any> {
...
async function callEngine(engineName: string, payload: any): Promise<any> {
...
export async function orchestrateIntent(req: IntentRequest): Promise<any> {
  ...
  const intentText = (
    req.intent?.toString() || 
    (req as any).payload?.intent || 
    (req as any).payload?.text ||
    (req as any).text ||
    (req as any).naturalLanguageIntent ||
  ...
  const industry = detectIndustry(enhancedReq);
  ...
  await Promise.allSettled([
    generateEngineResponse('identity', naturalLanguageIntent, industry, req, identityResultRaw)
      .then(r => {
        identityResult = r;
        if (!OLLAMA_ONLY) {
    ...
    generateEngineResponse('tenancy', naturalLanguageIntent, industry, req, tenancyResultRaw)
  ...
  await Promise.allSettled([
    generateEngineResponse('compliance', naturalLanguageIntent, industry, req, complianceResultRaw)
      .then(r => complianceResult = r).catch(() => {}),
    generateEngineResponse('policy', naturalLanguageIntent, industry, req, policyResultRaw)
      .then(r => policyResult = r).catch(() => {}),
    generateEngineResponse('risk', naturalLanguageIntent, industry, req, riskResultRaw)
  ...
  await Promise.allSettled([
    generateEngineResponse('quota', naturalLanguageIntent, industry, req, quotaResultRaw)
    ...
    generateEngineResponse('evidence', naturalLanguageIntent, industry, req, evidenceResultRaw)
  ...
  try {
    routingResult = await generateEngineResponse('routing', naturalLanguageIntent, industry, req, routingResultRaw);
  ...
  await Promise.allSettled([
    generateEngineResponse('integration', naturalLanguageIntent, industry, req, integrationResultRaw)
      .then(r => { 
        integrationResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] Integration: Ollama enhancement failed');
      }),
    generateEngineResponse('pricing', naturalLanguageIntent, industry, req, pricingResultRaw)
      .then(r => { 
        pricingResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] Pricing: Ollama enhancement failed');
      }),
    generateEngineResponse('versioning', naturalLanguageIntent, industry, req, versioningResultRaw)
      .then(r => { 
        versioningResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] Versioning: Ollama enhancement failed');
      }),
    generateEngineResponse('change', naturalLanguageIntent, industry, req, changeResultRaw)
      .then(r => { 
        changeResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] Change: Ollama enhancement failed');
      }),
    generateEngineResponse('ai', naturalLanguageIntent, industry, req, aiResultRaw)
      .then(r => { 
        aiResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] AI: Ollama enhancement failed');
      }),
    generateEngineResponse('simulation', naturalLanguageIntent, industry, req, simulationResultRaw)
      .then(r => { 
        simulationResult = r;
        if (!r.ollamaEnhanced) console.warn('[Orchestrator] Simulation: Ollama enhancement failed');
      })
  ]);
  ...
  try {
    explainResult = await generateEngineResponse('explainability', naturalLanguageIntent, industry, {
      ...req,
      complianceResult,
      policyResult,
      riskResult,
    ...
    if (explainResult.why && (
      explainResult.why.includes('buy a car') || 
  ...
  try {
    learningResult = await generateEngineResponse('learning', naturalLanguageIntent, industry, req, learningResultRaw);
  } catch {
  ...
  const normalizeDecision = (value?: string) => {
    ...
    if (upper === 'ALLOW' || upper === 'DENY' || upper === 'REVIEW') return upper;
    return undefined;
  };
  ...
  if (appealsEligible) {
    try {
      appealsResult = await generateEngineResponse('appeals', naturalLanguageIntent, industry, {
        ...req,
        finalDecision,
        explainResult
      }, appealsResultRaw);
  ...
  const summaryDetails = await generateSummaryDetails(naturalLanguageIntent, industry, req);
  ...
  const actionItems = await generateActionItems(naturalLanguageIntent, industry, req, allResultsSummary, status);
  const povOptions = await generatePovOptions(naturalLanguageIntent, industry);
  const sessionTypes = await generateSessionTypes(naturalLanguageIntent, industry);

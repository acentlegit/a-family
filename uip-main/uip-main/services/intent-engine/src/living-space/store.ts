...
import { EngineResult, IntentEnvelope, StateObject, GraphEdge } from "./engineRegistry";
...
export function getSession(id: string): IntentSession | null {
  return sessions.get(id) ?? null;
}

export function createSession(title?: string, intentType?: string) {
  const id = uid("intent");
...
export function addNoteEvent(id: string, by: string, text: string) {
...
function decisionToStatus(decision?: string) {
  ...
  if (upper === "ALLOW") return "pass";
  if (upper === "DENY") return "fail";
  if (upper === "REVIEW") return "warn";
  return undefined;
}

function riskToSeverity(level?: string) {
...
function finalDecisionToStatus(finalDecision?: string) {
  const upper = String(finalDecision ?? "").toUpperCase();
  if (upper === "ALLOW") return "approved";
  if (upper === "DENY") return "rejected";
  if (upper === "REVIEW" || upper === "UNKNOWN") return "needs_review";
  return undefined;
}

function buildEdges(objects: StateObject[]): GraphEdge[] {
...
function buildEngineEvent(engine: string, status?: string, summary?: string, details?: Record<string, any>): EngineResult {
  return {
    engine: engine as any,
    status: (status as any) ?? "warn",
    summary: summary ?? "No summary available",
    details,
  };
}

export async function executeSession(id: string, envelope: IntentEnvelope) {
  const session = sessions.get(id);
  if (!session) return null;
  session.intentType = envelope.intentType || session.intentType;
  session.events.push({ type: "IntentReceived", ts: now(), envelope });

  const orchestratorResult = await orchestrateIntent({
  ...
  const engineEvents: EngineResult[] = [];

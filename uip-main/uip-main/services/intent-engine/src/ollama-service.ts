...
function normalizeLibraryModel(model: string): string {
 ...
 */
function extractIntentName(intentText: string, engineName: string): string {
  ...
  const engineIntentMap: Record<string, string> = {
 ...
 */
function getCategoryForEngine(engineName: string, intentText?: string, industry?: string): string {
  ...
  const engineCategoryMap: Record<string, string> = {
 ...
 */
function getModelSpecificPrompt(model: string, category: string): string {
 ...
 */
function buildSystemPrompt(category: string, intent: string, model: string): string {
 ...
 */
function buildUserPrompt(intent: string, context?: string): string {
  const contextStr = context ? ` Context: ${context}` : "";
  return `Intent: ${intent}${contextStr}
Produce output strictly matching the JSON schema.
Requirements:
- Provide step-by-step workflow guidance (clear ordered steps).
- Use the exact intent context; do NOT invent unrelated facts.
- Be concrete and user-friendly so a non-technical user can understand.`;
}

function buildLibraryPrompt(intentName: string, context: string): string | null {
  try {
    return buildPrompt(INTENT_LIBRARY_PATH, intentName, normalizeLibraryModel(OLLAMA_MODEL), context);
  } catch (error: any) {
...
Provide ONLY valid JSON with real, specific values. NO placeholders, NO hardcoded examples.
Always include step-by-step guidance and explain decisions in plain language.
Analyze the actual intent provided and generate relevant, specific information.`;
    }
...
Return JSON only. Ensure next_steps is a clear ordered list of actions.`,
 ...
 */
function processOllamaResponse(
  response: string,
  engineName: string,
  intentText: string,
  industry: string,
  ...
  try {
    ...
    if (transformed.summary && typeof transformed.summary === 'string') {
      ...
      if (parsed.requirements && Array.isArray(parsed.requirements)) {
        parsed.requirements = parsed.requirements.map((r: any) => {
          if (typeof r === 'string') {
      ...
      if (parsed.policies && Array.isArray(parsed.policies)) {
        parsed.policies = parsed.policies.map((p: any) => {
          ...
          return p;
        }).filter((p: any) => p && typeof p === 'string' && p.length > 2 && !p.match(/^policy\s*\d+$/i));
        ...
        if (!OLLAMA_ONLY) {
          ...
          if (parsed.policies.length === 0) {
            ...
            const industry = (engineResult as any).industry || 'general';
      ...
      if (!OLLAMA_ONLY) {
        ...
        if (engineName === 'pricing') {
          ...
          if (!parsed.totalCost || typeof parsed.totalCost !== 'number') {
            parsed.totalCost = parsed.amount + (parsed.fees.reduce((sum: number, fee: any) => sum + (fee.amount || 0), 0));
          }
        }
      ...
      if (engineName === 'explainability') {
        ...
        if (parsed.why) {
          ...
          if (typeof parsed.why === 'object' && !Array.isArray(parsed.why)) {
            ...
            if (parsed.why.reasoning && typeof parsed.why.reasoning === 'string') textParts.push(parsed.why.reasoning);
            if (parsed.why.reason && typeof parsed.why.reason === 'string') textParts.push(parsed.why.reason);
            if (parsed.why.explanation && typeof parsed.why.explanation === 'string') textParts.push(parsed.why.explanation);
            if (parsed.why.why && typeof parsed.why.why === 'string') textParts.push(parsed.why.why);
            // If it has an array of factors, convert to readable text
            if (Array.isArray(parsed.why.reasoning)) {
              textParts.push(...parsed.why.reasoning.filter((r: any) => typeof r === 'string'));
            }
            if (Array.isArray(parsed.why.factors)) {
              const factors = parsed.why.factors.map((f: any) => {
                if (typeof f === 'string') return f;
                if (typeof f === 'object' && f.factor) return `${f.factor}: ${f.value || ''}`;
                return '';
            ...
            parsed.why = parsed.why.filter((r: any) => typeof r === 'string' || (typeof r === 'object' && r.factor))
              .map((r: any) => typeof r === 'string' ? r : `${r.factor}: ${r.value || ''}`)
              .join('. ');
          }
        } else {
      ...
      if (engineName === 'evidence') {
        ...
        if (parsed.documents && Array.isArray(parsed.documents)) {
          ...
          parsed.records = parsed.documents
            ...
            .filter((rec: any) => rec.name && rec.name.trim().length > 0);
          ...
          parsed.records = parsed.records
            ...
            .filter((rec: any) => rec && rec.name && rec.name.trim().length > 0);
        }
        // Remove any description, type, or other nested fields to keep it simple
        if (parsed.records && Array.isArray(parsed.records)) {
          parsed.records = parsed.records.map((rec: any) => ({ name: rec.name || String(rec) }));
        }
      }
      ...
      if (engineName === 'explainability') {
        ...
        let fees: Array<{ type: string; amount: number }> = [];
        if (feesMatch) {
        ...
        let requiredDocs: Array<{ type: string; description: string }> = [];
        if (docsMatch) {
        ...
        if (probMatch) {
          ...
          if (prob > 1) prob = prob / 100; // Convert percentage to decimal
 ...
 */
export async function generateSummaryDetails(
  intentText: string,
  industry: string,
  payload: any
): Promise<any> {
...
Return ONLY details that are explicitly present in the intent text. If a field is not mentioned, omit it. Return JSON only.`;
    ...
    try {
      const parsed = JSON.parse(jsonStr);
      return sanitizeSummaryDetails(intentText, parsed);
    } catch (parseError: any) {
...
function sanitizeSummaryDetails(intentText: string, summary: any) {
  if (!summary || typeof summary !== "object") return {};
  const intentLower = intentText.toLowerCase();
  const containsValue = (value: any) => {
    if (value == null) return false;
    if (typeof value === "number") {
      return intentLower.includes(String(value));
    }
    if (typeof value === "string") {
      const normalized = value.trim().toLowerCase();
      if (!normalized || normalized.length < 3) return false;
      return intentLower.includes(normalized);
    }
    if (typeof value === "object") {
      if (typeof value.id === "string") return containsValue(value.id);
      if (typeof value.name === "string") return containsValue(value.name);
    }
    return false;
  };
...
export async function generateActionItems(
  ...
  allResults: any,
  status: string
): Promise<Array<{ action: string; type: string; priority: string; description: string; url: string }>> {
  const prompt = `USER INTENT: "${intentText}"
INDUSTRY: ${industry}

ANALYZE THE USER'S INTENT: "${intentText}"

Generate a COMPLETE, STEP-BY-STEP plan from start to finish. Each action should:
1. Be a concrete task (book tickets, fill form, register, schedule appointment, etc.)
2. Be ordered in the exact sequence the user should follow
3. Use the specific details from the intent (locations, dates, budget, preferences)
4. Have a REAL website URL where the user can complete the action
5. Be directly related to what the user wants to accomplish
...
Return ONLY valid JSON array (no text before/after). The order of items is the required step order:
[
  {
...
4. Generate 5-9 relevant actions covering the full journey (start to finish)
5. Focus on WHAT the user needs to DO, not just information
6. Do NOT include steps for unrelated industries or topics
7. For travel, include steps from booking tickets to post-trip wrap-up and local highlights if requested
...
Generate practical actions with real URLs. Return JSON array only.`;
  ...
  try {
    ...
    if (!response) {
      ...
      const defaultActions: Array<{ action: string; type: string; priority: string; description: string; url: string }> = [];
      if (status === 'DENIED') {
        defaultActions.push({
    ...
    try {
      ...
      const actionsWithUrls = Array.isArray(actions) ? actions
        .filter((action: any) => {
          ...
          const isRetail = intentLower.includes('refund') || intentLower.includes('return') ||
                           intentLower.includes('order') || intentLower.includes('delivery') ||
                           intentLower.includes('replacement') || industry === 'retail';

          if (isRetail) {
            // For retail intents, reject travel/medical/vehicle actions
            if (actionText.includes('flight') || actionText.includes('hotel') || actionText.includes('visa') ||
                actionText.includes('passport') || actionText.includes('dentist') || actionText.includes('clinic') ||
                actionText.includes('doctor') || actionText.includes('vehicle') || actionText.includes('car')) {
              console.log(`[Action Filter] Rejected unrelated action for retail intent: ${action.action}`);
              return false;
            }
          }
        ...
        .filter((action: any) => !OLLAMA_ONLY || (action.url && String(action.url).trim().length > 0)) : [];
...
/**
 * Generate POV roles/stakeholders for the intent.
 */
export async function generatePovOptions(
  intentText: string,
  industry: string
): Promise<string[]> {
  const prompt = `USER INTENT: "${intentText}"
...
Generate a comprehensive list of stakeholders/roles who would be involved in this intent.
Return ONLY a JSON array of strings, 6-12 items.
Examples:
- Real estate: ["BUYER","SELLER","AGENT","BROKER","LENDER","INSURER","TITLE_COMPANY"]
- Healthcare: ["PATIENT","PRACTITIONER","HOSPITAL","PHARMACY","PAYER"]
- Travel: ["TRAVELER","AIRLINE","HOTEL","AGENT","IMMIGRATION","INSURER"]
If the intent implies additional parties (e.g., GOVERNMENT, PAYMENT_PROCESSOR, SUPPORT, COMPLIANCE, REVIEWER), include them.
...
Return JSON array only.`;
  ...
  try {
    ...
    const extracted = extractStringsFromParsed(parseOllamaJson(response), [
      "povOptions",
      "pov_options",
      "povertyOptions",
      "roles",
    ...
    if (!extracted.length && looksLikeSchema(response)) {
      const retryPrompt = `${prompt}

IMPORTANT: Return ONLY a JSON array of strings. Do NOT return a schema. No prose.`;
      const retryResponse = await callOllama(retryPrompt);
      const retryExtracted = extractStringsFromParsed(parseOllamaJson(retryResponse), [
        "povOptions",
        "pov_options",
        "povertyOptions",
        "roles",
    ...
    console.warn(`[Ollama] Failed to generate POV options: ${error.message}`);
    return [];
  }
}

/**
 * Generate session types for the intent (industry-specific workflow steps).
 */
export async function generateSessionTypes(
  intentText: string,
  industry: string
): Promise<string[]> {
  const prompt = `USER INTENT: "${intentText}"
...
Generate 3-6 short workflow step names (session types) for this intent.
They must be concise labels, title case, 1-2 words each.
Examples:
- Real estate: ["Property","Title","Escrow","Inspection"]
- Healthcare: ["Encounter","Coverage","Claim","MedicationRequest"]
- Travel: ["Itinerary","Flight","Hotel","Visa"]
- Retail: ["Order","Refund","Fraud","Customer"]
- Automobile: ["Ownership","Title","Registration","Recall"]
...
IMPORTANT: Return ONLY a JSON array of strings. Do NOT return a schema. No prose.`;
      ...
      return retryExtracted.map((item) => item.trim()).filter((item) => item.length > 0);
    }
    return extracted.map((item) => item.trim()).filter((item) => item.length > 0);
  } catch (error: any) {
    console.warn(`[Ollama] Failed to generate session types: ${error.message}`);
    return [];
  }
}

function looksLikeSchema(response: string): boolean {
...
function parseOllamaJson(response: string): any {
  let jsonStr = response.trim();
  const jsonMatch = response.match(/```json\n([\s\S]*?)\n```/);
  if (jsonMatch) {
    jsonStr = jsonMatch[1];
...
function extractStringsFromParsed(parsed: any, preferredKeys: string[]): string[] {
  const items: string[] = [];
  const pushItem = (value: any) => {
    ...
    if (value && typeof value === "object") {
      if (typeof value.name === "string") items.push(value.name);
      if (typeof value.title === "string") items.push(value.title);
      if (typeof value.step === "string") items.push(value.step);
      if (typeof value.role === "string") items.push(value.role);
      if (typeof value.type === "string") items.push(value.type);
    }
  };
  ...
  if (Array.isArray(parsed)) {
    ...
    if (!items.length && Array.isArray((parsed as any).items)) {
      (parsed as any).items.forEach(pushItem);
    }
  }

  return Array.from(new Set(items.map((item) => String(item).trim()))).filter(
    (item) => item.length > 0
  );
}
 ...
 */
function generateDefaultUrl(action: string, industry: string): string {
  if (OLLAMA_ONLY) return '';
  ...
  if (lowerAction.includes('passport')) return 'https://travel.state.gov/content/travel/en/passports.html';
  if (lowerAction.includes('visa')) return 'https://travel.state.gov/content/travel/en/us-visas.html';
  ...
  if (lowerAction.includes('insurance') && industry === 'healthcare') return 'https://www.healthcare.gov/';
  if (lowerAction.includes('consent') || lowerAction.includes('authorization')) return 'https://www.hhs.gov/hipaa/index.html';
  
  // Vehicle related
  if (lowerAction.includes('emission') || lowerAction.includes('smog')) return 'https://www.dmv.org/emissions-testing.php';
  ...
  if (lowerAction.includes('insurance') && industry === 'automobile') return 'https://www.usa.gov/auto-insurance';
  ...
  if (lowerAction.includes('document') || lowerAction.includes('submit')) return 'https://www.usa.gov/';
  
  // Real estate related
  if (lowerAction.includes('zoning') || lowerAction.includes('permit')) return 'https://www.usa.gov/local-governments';
  
  // Finance related
  if (lowerAction.includes('kyc') || lowerAction.includes('verification')) return 'https://www.usa.gov/banks';
  
  // Default based on industry
  if (industry === 'healthcare') return 'https://www.healthcare.gov/';
  if (industry === 'automobile') return 'https://www.dmv.org/';
  if (industry === 'real-estate') return 'https://www.usa.gov/local-governments';

...
export interface IntentLibraryEntry {
  category: string;
  intent: string;
  systemPromptLlama3?: string;
  systemPromptMistral?: string;
  systemPromptCodeLlama?: string;
  developerPrompt: string;
  userPromptTemplate: string;
  jsonOutputSchema: string;
}

export interface IntentLibrary {
  intents: IntentLibraryEntry[];
}
 ...
 */
function loadIntentLibrary(filePath: string): IntentLibrary {
 ...
 */
function loadIntentLibraryFromJSON(filePath: string): IntentLibrary {
  try {
 ...
 */
function loadIntentLibraryFromExcel(filePath: string): IntentLibrary {
  try {
    ...
    const intents: IntentLibraryEntry[] = data.map((row: any) => ({
 ...
 */
export function getIntentEntry(
  library: IntentLibrary,
  intentName: string
): IntentLibraryEntry | null {
  return library.intents.find(entry => 
    entry.intent.toLowerCase() === intentName.toLowerCase()
  ) || null;
}
...
export function getIntentsByCategory(
  ...
  return library.intents.filter(entry =>
 ...
 */
export function loadIntentLibraryData(filePath: string): IntentLibrary {
